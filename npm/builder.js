...
function Builder () {
  if (process.env.PRAWN_VERSION) {
    this.prawnVersion = process.env.PRAWN_VERSION;
  } else {
    this.prawnVersion = '2.1.0'; // or 'master'
  }
}
...
Builder.prototype.build = function (callback) {
  ...
  async.series([
    ...
    callback => builder.downloadDependencies('prawnpdf', 'prawn', builder.prawnVersion, callback), // download dependencies
    callback => builder.downloadDependencies('prawnpdf', 'ttfunk', '1.5.1', callback), // download dependencies
    callback => builder.downloadDependencies('prawnpdf', 'pdf-core', '0.7.0', callback), // download dependencies
...
Builder.prototype.uglify = function (callback) {
  ...
  const tasks = [
    {source: 'build/prawn.js', destination: 'build/prawn.min.js' }
  ].map(file => {
...
Builder.prototype.compile = function (callback) {
  ...
  new OpalCompiler({dynamicRequireLevel: 'ignore', defaultPaths: ['build/prawn/lib', 'build/ttfunk/lib', 'build/pdf-core/lib']})
    .compile('prawn', 'build/prawn-lib.js');
//  new OpalCompiler({dynamicRequireLevel: 'ignore', defaultPaths: ['build/ttfunk/lib']})
//    .compile('ttfunk', 'build/ttfunk-lib.js');
